...
async def place_immediate_market_order(account, trade):
    connection = account.get_rpc_connection()
    print("Connecting to MetaApi RPC for immediate market order...")
    await connection.connect()
    print("Connected to MetaApi RPC for immediate market order, waiting for synchronization...")
    await connection.wait_synchronized()
    print("Synchronized with MetaApi for immediate market order")
...
async def close_all_open_trades(account):
    connection = account.get_rpc_connection()
    print("Connecting to MetaApi RPC to close open positions...")
    await connection.connect()
    print("Connected to MetaApi RPC, waiting for synchronization to close open positions...")
    await connection.wait_synchronized()
    print("Synchronized with MetaApi for closing open positions")
